...
1. **Derived validation gates.** Canonical discovery leaves several questions explicitly open without assigning them an identifier. Rather than silently attaching them to an unrelated gate or inventing scope, three gates are declared with `origin: DERIVED` and a written derivation rationale: `JV-MANUAL-CHANNELS`, `JV-PAYMENT-PROCEDURE`, and `JV-INCIDENT-FAIRNESS`. The validator refuses a derived gate that lacks a rationale.
